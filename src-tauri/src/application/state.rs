...
impl AppState {
    pub fn insert(&self, document: ImageDocument) -> Result<(), String> {
        let mut documents = self.documents.write().map_err(|_| "No se pudo bloquear el registro de documentos".to_string())?;
    ...
    pub fn get(&self, id: &str) -> Result<Arc<Mutex<ImageDocument>>, String> {
        let documents = self.documents.read().map_err(|_| "No se pudo leer el registro de documentos".to_string())?;
        documents.get(id).cloned().ok_or_else(|| "Documento no encontrado".to_string())
    }
}
