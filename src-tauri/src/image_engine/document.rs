...
impl ImageDocument {
    pub fn decode(id: String, name: String, bytes: Vec<u8>) -> Result<(Self, ImportedDocument), String> {
        let image_format = image::guess_format(&bytes).map_err(|error| format!("No se pudo identificar el formato: {error}"))?;
        ...
        let is_sixteen = matches!(decoded.color(), ColorType::L16 | ColorType::La16 | ColorType::Rgb16 | ColorType::Rgba16);
    ...
    pub fn analyze(&mut self) -> AlphaAnalysis {
        let analysis = alpha_engine::analyze(&self.id, self.revision, self.width, self.height, &self.working);
    ...
    pub fn apply_treatment(&mut self, treatment: &AlphaTreatment) -> TreatmentResult {
        ...
        let delta = alpha_engine::apply_treatment(&mut self.working, self.width, self.height, treatment);
        self.revision += 1;
        self.history.push(HistoryEntry { label: treatment.label().into(), delta });
        self.future.clear();
        let analysis = self.analyze();
        TreatmentResult { revision: self.revision, impact, analysis }
    }
    ...
    pub fn redo(&mut self) -> bool {
        let Some(entry) = self.future.pop() else { return false };
    ...
    pub fn preview_png(&self, mode: &str) -> Result<Vec<u8>, String> {
        let rgba = alpha_engine::preview_rgba8(
            if mode == "original" { &self.original } else { &self.working },
            mode,
        );
    ...
    pub fn export_verified(&self, path: &Path, require_solid_alpha: bool, dpi: u32) -> Result<ExportVerification, String> {
        let before = alpha_engine::analyze(&self.id, self.revision, self.width, self.height, &self.working);
        if require_solid_alpha && !before.verified_solid_alpha {
            return Err(format!("EXPORT_BLOCKED_PARTIAL_ALPHA: quedan {} píxeles semitransparentes", before.partial_alpha_pixels));
        }
        let encoded = self.encode_png(dpi)?;
        std::fs::write(path, &encoded).map_err(|error| format!("No se pudo guardar el PNG: {error}"))?;
        let reopened = std::fs::read(path).map_err(|error| format!("No se pudo reabrir el PNG exportado: {error}"))?;
        let (mut verification_document, imported) = ImageDocument::decode("export_verification".into(), "export.png".into(), reopened)?;
        ...
        if require_solid_alpha && !verification.verified_solid_alpha {
            return Err(format!("EXPORT_VERIFICATION_FAILED: se reabrió con {} píxeles semitransparentes", verification.partial_alpha_pixels));
        }
    ...
    fn encode_png(&self, dpi: u32) -> Result<Vec<u8>, String> {
        let mut bytes = Vec::new();
        {
            let mut encoder = png::Encoder::new(&mut bytes, self.width, self.height);
            encoder.set_color(png::ColorType::Rgba);
            encoder.set_depth(if self.working.bit_depth() == 16 { png::BitDepth::Sixteen } else { png::BitDepth::Eight });
            ...
            let mut writer = encoder.write_header().map_err(|error| error.to_string())?;
            match &self.working {
                PixelBuffer::Rgba8(pixels) => writer.write_image_data(pixels).map_err(|error| error.to_string())?,
                PixelBuffer::Rgba16(pixels) => {
                    let encoded_pixels: Vec<u8> = pixels.iter().flat_map(|value| value.to_be_bytes()).collect();
                    writer.write_image_data(&encoded_pixels).map_err(|error| error.to_string())?;
                }
            }
        }
        Ok(bytes)
...
mod tests {
    ...
    fn png16_critical_alpha() -> Vec<u8> {
        ...
        let pixels: Vec<u8> = values.into_iter().flat_map(|alpha| [1000u16, 2000, 3000, alpha]).flat_map(u16::to_be_bytes).collect();
    ...
    fn decodes_real_png8_and_preserves_original_bytes() {
        ...
        let (mut document, imported) = ImageDocument::decode("doc8".into(), "fixture8.png".into(), bytes).unwrap();
        assert_eq!(imported.bit_depth, 8);
        assert_eq!(document.analyze().partial_alpha_pixels, 254);
        document.apply_treatment(&AlphaTreatment::Threshold { threshold: 128, reconstruct_radius: 2 });
    ...
    fn decodes_real_png16_without_reducing_depth() {
        let (mut document, imported) = ImageDocument::decode("doc16".into(), "fixture16.png".into(), png16_critical_alpha()).unwrap();
    ...
    fn overlay_preview_marks_only_partial_alpha_magenta() {
        let (document, _) = ImageDocument::decode("preview".into(), "fixture.png".into(), png8_all_alpha()).unwrap();
    ...
    fn exports_reopens_and_verifies_solid_alpha_without_touching_source() {
        ...
        let (mut document, _) = ImageDocument::decode("export".into(), "fixture.png".into(), bytes).unwrap();
        document.apply_treatment(&AlphaTreatment::Threshold { threshold: 128, reconstruct_radius: 2 });
    ...
    fn blocks_solid_export_when_partial_alpha_remains() {
        let (document, _) = ImageDocument::decode("blocked".into(), "fixture.png".into(), png8_all_alpha()).unwrap();
